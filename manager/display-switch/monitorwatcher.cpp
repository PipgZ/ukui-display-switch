...
MonitorWatcher* MonitorWatcher::_instance = nullptr;

MonitorWatcher* MonitorWatcher::instance(QObject* parent)
{
    QMutex mutex;
    mutex.lock();
    if(_instance == nullptr)
        _instance = new MonitorWatcher(parent);
    mutex.unlock();
    return _instance;
}
...
void MonitorWatcher::run()
{
    ...
    while(!isInterruptionRequested()) {
        ...
        if(tmp != monitorCount){
            ...
            qDebug() << "info: [Class:MonitorWatcher][run]: Monitor Changed!";
        }
        sleep(3);
    }
}
